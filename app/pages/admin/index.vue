...
<template>
  <div class="space-y-8 pb-12">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
      <!-- Recent Projects -->
      <UCard :ui="{ body: 'p-0', rounded: 'rounded-2xl' }" class="border-slate-200 dark:border-slate-800 overflow-hidden">
        <template #header>
          <div class="flex items-center justify-between p-2">
            ...
            <UButton to="/admin/projects" variant="link" color="teal" size="sm">View All</UButton>
          </div>
        </template>
        ...
        <div v-else-if="recentProjects.length > 0" class="divide-y divide-slate-100 dark:divide-slate-800">
          <div v-for="proj in recentProjects" :key="proj.id" class="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors flex items-center gap-4">
            ...
            <UButton :to="`/admin/projects/${proj.id}`" variant="ghost" icon="i-heroicons-pencil-square" color="gray" size="sm" />
          </div>
        </div>
        <div v-else class="text-center py-12 text-slate-500">
          <UIcon name="i-heroicons-folder-open" class="w-12 h-12 mx-auto mb-2 opacity-20" />
          <p>No projects yet.</p>
          <UButton to="/admin/projects/new" variant="link" color="teal" class="mt-2">Add your first project</UButton>
        </div>
      </UCard>
      ...
      <UCard :ui="{ body: 'p-0', rounded: 'rounded-2xl' }" class="border-slate-200 dark:border-slate-800 overflow-hidden">
        <template #header>
          <div class="flex items-center justify-between p-2">
            ...
            <UButton to="/admin/blog" variant="link" color="blue" size="sm">View All</UButton>
          </div>
        </template>
          ...
          <div v-for="post in recentPosts" :key="post.id" class="p-4 hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors flex items-center gap-4">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex items-center gap-2 mt-1">
                <UBadge :color="post.published ? 'teal' : 'gray'" size="xs" variant="soft">{{ post.published ? 'Published' : 'Draft' }}</UBadge>
                <span class="text-[10px] text-slate-500">{{ new Date(post.created_at).toLocaleDateString() }}</span>
              </div>
            </div>
            <UButton :to="`/admin/blog/${post.id}`" variant="ghost" icon="i-heroicons-pencil-square" color="gray" size="sm" />
          </div>
        </div>
        <div v-else class="text-center py-12 text-slate-500">
          <UIcon name="i-heroicons-document-minus" class="w-12 h-12 mx-auto mb-2 opacity-20" />
          <p>No articles yet.</p>
          <UButton to="/admin/blog/new" variant="link" color="blue" class="mt-2">Write your first post</UButton>
        </div>
      </UCard>
    </div>
  </div>

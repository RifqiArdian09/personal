...
const links = computed(() => [
  {
    label: locale.value === "id" ? "Beranda" : "Home",
    to: "/",
    hash: "#home",
    icon: "i-heroicons-home",
  },
  {
    label: locale.value === "id" ? "Tentang" : "About",
    to: "/",
    hash: "#about",
    icon: "i-heroicons-user",
  },
  {
    label: locale.value === "id" ? "Portofolio" : "Portfolio",
    to: "/",
    hash: "#projects",
    icon: "i-heroicons-code-bracket-square",
  },
  {
    label: locale.value === "id" ? "Sertifikat" : "Certificates",
    to: "/",
    hash: "#certificates",
    icon: "i-heroicons-trophy",
  },
  {
    label: locale.value === "id" ? "Pertanyaan" : "Questions",
    to: "/",
    hash: "#faq",
...
const openWhatsApp = () => {
  ...
  const text = encodeURIComponent(
    "Halo Rifqi! Saya baru saja melihat portofolio Anda.",
  );
...
<template>
  <div
    class="min-h-screen bg-surface text-text-primary font-body selection:bg-accent selection:text-white bg-noise"
  >
    <!-- Scroll Progress Bar -->
    ...
    >
      ...
      >
        {{ waLabelText
        }}<span class="ml-1 w-1 h-3 bg-accent animate-pulse"></span>
      </span>
      ...
      >
        ...
        <!-- Glowing Ring Effect -->
        <div
          class="absolute -inset-1 rounded-full bg-[#25D366]/40 blur-md -z-10 animate-pulse"
        ></div>
      </button>
    </div>
    ...
    <header
      ...
      :class="
        scrolled
          ? 'bg-surface/70 backdrop-blur-2xl border-b-[2px] border-manga-border shadow-[0_4px_20px_rgba(0,0,0,0.05)] py-2'
          : 'bg-transparent py-4'
      "
    >
      <div
        class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 h-14 flex items-center justify-between"
      >
        <!-- Logo -->
        <NuxtLink to="/" class="flex items-center gap-3 group relative">
          <div
            class="absolute -inset-2 bg-accent/20 blur-xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500"
          ></div>
          <span
            class="font-black font-display text-lg sm:text-2xl tracking-tighter relative z-10 flex items-center transition-transform group-hover:scale-105"
          >
            <span
              class="text-accent opacity-50 group-hover:opacity-100 transition-opacity mr-1"
              >{</span
            >
            <span>Rifqi</span><span class="text-accent">.dev</span>
            <span
              class="text-accent opacity-50 group-hover:opacity-100 transition-opacity ml-1"
              >}</span
            >
          </span>
        </NuxtLink>
        ...
        <!-- Desktop Nav -->
        <nav
          class="hidden lg:flex items-center gap-0.5 bg-surface-alt/50 border-2 border-manga-border rounded-xl p-1 shadow-[3px_3px_0px_var(--color-manga-border)]"
        >
          <NuxtLink
            v-for="link in links"
            :key="link.label"
          ...
          >
            <div class="relative z-10 flex items-center gap-2">
              <UIcon
                :name="link.icon"
                class="w-4 h-4 transition-all duration-500 group-hover:scale-125 group-hover:-rotate-6"
              />
            ...
            <!-- Hover Effect Pill -->
            <div
              class="absolute inset-0 bg-accent/10 rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 scale-90 group-hover:scale-100"
            ></div>
          </NuxtLink>
        </nav>
        ...
        <div class="flex items-center gap-4">
          ...
          >
            ...
            />
            <span class="font-black font-mono text-[7px] uppercase mt-0.5">{{
              locale === "id" ? "EN" : "ID"
            }}</span>
          </button>
    ...
    <Teleport to="body">
      ...
      <Transition name="modal">
        <div
          v-if="isMobileMenuOpen"
          class="fixed top-0 right-0 bottom-0 w-[280px] bg-surface border-l-[4px] border-manga-border z-[999] p-6 flex flex-col shadow-2xl"
        >
          <div
            class="flex justify-between items-center mb-8 pb-4 border-b-2 border-divider"
          >
            <span class="font-bold font-display text-lg text-accent">Menu</span>
            <button
              @click="isMobileMenuOpen = false"
              class="w-10 h-10 rounded-xl border-2 border-manga-border flex items-center justify-center cursor-pointer hover:bg-surface-alt transition-colors"
            >
          ...
          <div
            class="mt-auto pt-8 border-t-2 border-divider flex justify-center gap-4"
          >
            <a
            ...
            <a
              href="https://linkedin.com/in/rifqiardian"
              target="_blank"
              class="w-11 h-11 rounded-xl border-2 border-manga-border shadow-[3px_3px_0px_var(--color-manga-border)] flex items-center justify-center hover:-translate-y-1 transition-all bg-surface"
            >
    ...
    <!-- Footer -->
    <footer
      class="bg-surface border-t-[4px] border-manga-border relative overflow-hidden"
    >
      <!-- Decorative background -->
      <div
        class="absolute inset-0 opacity-[0.03] pointer-events-none bg-hero-pattern"
      ></div>

      <div
        class="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-24 relative z-10"
      >
        <div
        ...
        >
          ...
          <div class="space-y-6 lg:col-span-1">
            <NuxtLink to="/" class="flex items-center gap-3 group">
              <span class="font-black font-display text-2xl tracking-tight"
                >{ Rifqi<span class="text-accent">.dev }</span></span
              >
            </NuxtLink>
            <p
              class="text-text-secondary text-sm font-medium leading-relaxed max-w-xs"
            >
              Membangun pengalaman digital premium dengan fokus pada performa,
              aksesibilitas, dan estetika modern.
            </p>
            <div class="flex items-center gap-4">
              <a
                v-for="social in [
                  ...
                  },
                  {
                    ...
                    url: 'https://linkedin.com/in/rifqiardian',
                  },
                  {
                    icon: 'i-simple-icons-instagram',
                    url: 'https://instagram.com/rifqiardian_09',
                  },
                ]"
                :key="social.icon"
                :href="social.url"
          ...
          <div class="space-y-6">
            <h4
              class="font-black font-display text-xs uppercase tracking-[0.2em] text-text-primary border-l-4 border-accent pl-4"
            >
              Navigasi
            </h4>
            <nav class="flex flex-col gap-3">
              ...
              >
                <span
                  class="w-0 group-hover:w-3 h-[2px] bg-accent transition-all"
                ></span>
                {{ link.label }}
              </NuxtLink>
            </nav>
          ...
          <div class="space-y-8">
            <div class="space-y-4">
              ...
              <div
                class="inline-flex items-center gap-2 px-3 py-1.5 bg-green-500/10 text-green-500 text-[10px] font-black uppercase tracking-widest rounded-lg border border-green-500/20"
              >
                <span
            ...
            <div class="space-y-4">
              ...
              <div class="flex flex-wrap gap-2">
                <span
                  v-for="skill in ['Web Dev', 'UI/UX', 'Mobile', 'Security']"
                  :key="skill"
        ...
        <!-- Bottom Bar -->
        <div
          class="mt-20 pt-8 border-t-2 border-divider flex flex-col md:flex-row items-center justify-between gap-6"
        >
          <p
            class="text-[10px] font-black font-mono text-text-secondary uppercase tracking-widest"
          >
            &copy; {{ new Date().getFullYear() }} —
            <span class="text-text-primary">Rifqi Ardian</span>. All Rights
            Reserved.
          </p>
        </div>
      </div>
